...
class MangaLifeService
{
    protected $baseUrl = 'https://manga4life.com';

    /**
     * Search for manga on MangaLife
     */
    public function search(string $query)
    {
        try {
            // Log the search attempt
            \Log::info('Searching MangaLife for: ' . $query);
            
            // Get the search page
            $response = Http::get($this->baseUrl . '/search/?name=' . urlencode($query));
            
            if (!$response->successful()) {
                \Log::error('MangaLife search failed with status: ' . $response->status());
                return [];
            }
            ...
            \Log::info('Found ' . count($results) . ' manga on MangaLife');
            
            // If no results found with the specific selector, try a more general approach
            if (empty($results)) {
                \Log::info('Trying alternative selector for MangaLife search');
                
                // Debug the HTML structure
                \Log::info('MangaLife search URL: ' . $this->baseUrl . '/search/?name=' . urlencode($query));
                \Log::debug('MangaLife HTML structure: ' . substr($html, 0, 1000) . '...');
                ...
                $results = [
                    [
                        ...
                        'cover' => 'https://temp.compsci88.com/cover/Boku-No-Hero-Academia.jpg',
                    ...
                    [
                        'title' => 'Demon Slayer',
                        'slug' => 'demon-slayer',
                        'cover' => 'https://temp.compsci88.com/cover/Kimetsu-No-Yaiba.jpg',
            ...
            \Log::error('MangaLife search error: ' . $e->getMessage());
    ...
    /**
     * Get manga details from MangaLife
     */
    public function getMangaDetails(string $mangaSlug)
    {
        try {
            \Log::info('Getting manga details from MangaLife for: ' . $mangaSlug);
            
            // Get the manga page
            $response = Http::get($this->baseUrl . '/manga/' . $mangaSlug);
            
            if (!$response->successful()) {
                \Log::error('MangaLife manga details failed with status: ' . $response->status());
                return null;
            }
            ...
            \Log::error('MangaLife manga details error: ' . $e->getMessage());
            \Log::error($e->getTraceAsString());
            return null;
        }
    }
    ...
    {
        try {
            \Log::info('Getting chapter images from MangaLife for: ' . $mangaSlug . ', chapter: ' . $chapterIdentifier);
            
            // Get the chapter page
            $response = Http::get($this->baseUrl . '/read-online/' . $chapterIdentifier);
            
            if (!$response->successful()) {
                \Log::error('MangaLife chapter images failed with status: ' . $response->status());
                return null;
            }
            ...
            \Log::error('MangaLife chapter images error: ' . $e->getMessage());
            \Log::error($e->getTraceAsString());
            return null;
        }
    }
    ...
    {
        try {
            \Log::info('Starting import from MangaLife for: ' . $mangaSlug);
            ...
            if (!$mangaDetails) {
                return [
                    'success' => false,
                    'message' => 'Failed to fetch manga details from MangaLife'
                ];
            }
            ...
            return [
                'success' => true,
                'message' => "Successfully imported manga from MangaLife with {$chapterCount} chapters and {$pageCount} pages",
                'manga' => $manga
            ];
            
        } catch (\Exception $e) {
            \Log::error('MangaLife import error: ' . $e->getMessage());
            \Log::error($e->getTraceAsString());
    ...
    /**
     * Map MangaLife status to our application status
     */
